...
$sql = "SELECT r.ID_Reposicao, r.DataReposicao, r.Mensagem, r.docs_plano_aula, r.Status_Pedido, 
        ...
        JOIN professores p ON a.ID_Professor = p.ID_Professor"; // Ajuste os IDs de acordo com a sua estrutura de banco
        ...
        <tbody>
            <?php foreach ($pedidos as $pedido): ?>
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($pedido['Materia']); ?></td> <!-- Matéria -->
                    <td><?php echo htmlspecialchars($pedido['Professor']); ?></td> <!-- Nome do Professor -->
                    <td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.abrir-modal').on('click', function() {
                ...
                $('#id_reposicao').val(id);
                $('#status').val(status === 'Pendente' ? 'Aprovado' : status); // Alterar o status apenas se for pendente
                $('#modal').show();
            });
